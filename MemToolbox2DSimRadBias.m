function MemToolbox2DSimRadBias(nIter)
% MemToolbox2DSimRadBias(nIter)
% simulation with radial response biases towards the screen centre, and bias correction
%
...
parfor j=1:nSteps
    disp(j)
        ...
        for i=1:nParSets
            ...
            diffs = resps - dimensions'./2; % distance to screen centre on each response
...
t = toc
save('Data/MemToolbox2DSimRadBias.mat')
%%
...
% saveas(figure(2), 'Figs/MemToolbox2DSimRadBias_1.jpg');
%%
figure(3)
for j = 1:nSteps
    for i=1:4
...
% saveas(figure(3), 'Figs/MemToolbox2DSimRadBias_2.jpg');
%%
figure(4)
for j = 1:nSteps
    for i=1:4
...
% saveas(figure(4), 'Figs/MemToolbox2DSimRadBias_3.jpg');
...
saveas(figure(5), 'Figs/MemToolbox2DSimRadBias_4.jpg');
...
for j=1:2
    for i = 1:4
       ...
       xlim([1 11])
       if i==1; ylim([-80 80]); set(gca,'YTick',-80:40:80);
       else; ylim([-.1 .1]); set(gca,'YTick', -.1:.1:.1);end;
    end
end
subplot(2,4,5), colorbar(gca,'North','Ticks',[0 1],'TickLabels',[bias(1) bias(end)])
...
% saveas(figure(6), 'Figs/MemToolbox2DSimRadBias_5.jpg');
...
%     
% saveas(figure(7), 'Figs/MemToolbox2DSimRadBias_6.jpg');
...
for j=1:2
    for i = 1:4
       ...
       xlim([1 11])
       if i==1
           ylabel(sprintf('%s\nrecovery error',modelNames{j}),'FontWeight','bold')
       end
    end
end
...
h.XLabel.Visible = 'on';
xlabel('simulated parameter','FontWeight','normal')
% saveas(figure(8), 'Figs/MemToolbox2DSimRadBias_7.jpg');
